...
//        model.addAttribute("stores", null);
    ...
    public String addStore(Store store) {
        //Using store repository & save method
        stores.save(store);
        return "redirect:/";
    }
...
//        model.addAttribute("products", null);
        Store store = stores
                .findById(id) // coming from CRUD repository
                .orElse(null); //from the Optional If a value is present, returns the value, otherwise returns other.
    ...
    public String addProduct(Product product, HttpServletRequest request) {
        // put a row into the product table, which store the product happens to be in
        int storeId = Integer.parseInt(request.getParameter("storeId"));
        Store store = stores.findById(storeId).orElse(null);
        product.setStore(store);
        //product repository to save
    ...
    public String deleteProduct(Integer id, Integer storeId) {
        // methods from the CRUDRepository
